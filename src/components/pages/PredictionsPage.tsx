    ...
    return (
        <div className="min-h-[70vh] py-12 px-4" style={{ backgroundColor: '#333333' }}>
            <div className="max-w-[120rem] mx-auto">
                <h1 className="text-4xl font-heading font-bold text-white mb-6">Predictions</h1>
                ...
                <DragDropContext onDragEnd={onDragEnd}>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
                        {GROUP_IDS.map((g) => (
                            <div key={g} className="rounded-3xl bg-[#242424] p-5 shadow-sm">
                                <h2 className="text-xl font-semibold text-white mb-3">Group {g}</h2>
                                    ...
                                    <Droppable droppableId={`pool:${g}:0`} direction="vertical" isDropDisabled={finalMode}>
                                        {(provided, snapshot) => (
                                            <div
                                                ...
                                                className={`mb-4 min-h-[70px] rounded-xl border border-dashed p-3 ${snapshot.isDraggingOver ? 'border-purple-500/50 bg-purple-500/10' : 'border-white/20'
                                                    }`}
                                            >
                                                    ...
                                                    >
                                                        {(prov, snapshot) => (
                                                            <div
                                                                ...
                                                                className={`flex items-center justify-between rounded-lg card p-2 shadow-sm transition ${snapshot.isDragging ? 'opacity-80' : ''
                                                                    }`}
                                                            >
                                                                {renderTeam(team)}
                                                            </div>
                                        ...
                                        <Droppable key={idx} droppableId={`slot:${g}:${idx}`} isDropDisabled={finalMode}>
                                            {(provided, slotSnap) => (
                                                <div
                                                    ref={provided.innerRef}
                                                    {...provided.droppableProps}
                                                    className={`relative h-[72px] rounded-xl border px-4 transition-colors ${slotSnap.isDraggingOver
                                                            ? 'border-purple-500/60 bg-purple-500/10'
                                                            : 'border-white/10 bg-[#242424]'
                                                        }`}
                                                >
                                                    <div className="absolute inset-0 flex items-center gap-3 px-4">
                                                        <div className="w-7 shrink-0 font-semibold text-white/70">
                                                            {idx + 1}.
                                                        </div>
                                                        <div className="flex-1">
                                                            {slotVal ? (
                                                                ...
                                                                >
                                                                    {(prov, dragSnap) => (
                                                                        <div
                                                                            ...
                                                                            {...prov.draggableProps}
                                                                            {...prov.dragHandleProps}
                                                                            className={`flex h-[44px] items-center justify-between rounded-lg card px-3 py-2 shadow-sm transition ${dragSnap.isDragging ? 'opacity-80' : ''
                                                                                }`}
                                                                        >
                                                                            {renderTeam(slotVal)}
                                                                        </div>
                                                                ...
                                                                <div className="flex h-[44px] items-center text-sm text-white/50">
                                                                    (empty)
                                                                </div>
                                                            )}
                                                        </div>
                ...
                {!finalMode && (
                    <div className="mt-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <button
                            className="rounded-full bg-purple-600 px-6 py-3 text-sm font-semibold text-white shadow hover:bg-purple-700"
                            onClick={fillRandom}
                        >
                            Fill with random predictions
                        </button>
                        <button
                            className="rounded-full btn-primary px-6 py-3 text-sm font-semibold text-white shadow disabled:opacity-50"
